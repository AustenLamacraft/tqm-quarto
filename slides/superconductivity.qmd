...
$$
\overbrace{H_{\text{pair}}-\mu N}^{\equiv H_\mu}=2\sum_\bp \overbrace{\xi({\bp})}^{\xi_\bp\equiv\epsilon(\bp)-\mu}S_\bp^z+\frac{U_0}{V}\sum_{\bp,\bp'}S^+_\bp S^-_{\bp'}
$$
...
- Regard as approximate variational excited state, since orthogonal to $\ket{\text{BCS}}$

- If we chose $s=\uparrow$ then $\left(\bp,\uparrow\right)$ state is occupied and $\left(-\bp,\downarrow\right)$ empty
$$
\adop_{\bp
