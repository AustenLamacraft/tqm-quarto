...
- Take short-ranged interactions for simplicity $U(\br-\br') = U_0\delta(\br-\br')$
...
$$
Thus, $\mu$ is identified with the chemical potential
...
![](../assets/can.png){width=70% fig-align="center"}
...
$$
This is the __Onsager--Feynmann quantization condition__
...
- Parameterize $\varphi(r,\theta) = \sqrt{n} f(r/\xi)e^{i\ell\theta}$ to give an equation in $s\equiv r/\xi$ (set $\mu=U_0 n$ as before).
...
![](../assets/3dLattices.jpg){width=50% fig-align="center"}
...
\begin{align*}
H_\text{pair} = &N\epsilon(0)+\frac{U_0}{2V}N(N-1) 
+\sum_{\bk\neq 0}\overbrace{\left[\epsilon(\bk)-\epsilon(0)\right]}^{\equiv\tilde\epsilon(\bk)}\adop_\bk\aop_\bk\\
&+\frac{U_0}{2V}\sum_{\bk\neq 0}\left[\adop_{\bk}\adop_{-\bk}\aop_{0}\aop_{0} + \adop_{0}\adop_{0}\aop_{\bk}\aop_{-\bk}+2\adop_\bk\adop_0\aop_0\aop_\bk\right]
\end{align*}
$$
...
\end{align*}
$$
What happens if $\delta>\epsilon$? What changes if $\begin{pmatrix}
\epsilon & \delta \\\\
\delta & \epsilon
...
\tanh2\kappa_\bp=\frac{n_0 U_0}{\tilde\epsilon(\bk)+n_0 U_0}
\end{align*}
$$
...
$$
\omega(\bp) = \sqrt{\tilde\epsilon(\bk)\left(\tilde\epsilon(\bk)+2U_0n_0\right)}
$$

- Ground state energy is

$$
E_0=\frac{1}{2}nU_0  N+\sum_{\bp\neq 0}\frac{1}{2}\left[\omega(\bp)-\tilde\epsilon(\bk)-n_0U_0\right]
$$
...
$$
\omega(\bp)\underset{|\bp| \to \infty}{\longrightarrow} \tilde\epsilon(\bk) + n_0 U_0 -\frac{(n_0 U_0)^2}{2\tilde\epsilon(\bk)}
$$
...
$$
\begin{align*}
E_0&=\overbrace{\frac{1}{2}nU_0  N\left[1-\frac{1}{V}\sum_\bp \frac{U_0}{2\tilde\epsilon(\bk)}\right]}^{\text{1st and 2nd order PT}}\\
&+\overbrace{\sum_{\bp\neq 0}\frac{1}
{2}\left[\omega(\bp)-\tilde\epsilon(\bk)-n_0U_0+ \frac{(n_0U_0)^2}{2\tilde\epsilon(\bk)}\right]}^{\text{finite}}
\end{align*}
$$

- Term added and subtracted is contribution to the ground state energy in second order perturbation theory

- See [Appendix](../notes/bose-gas.qmd#regularizing-the-ground-state-energy) and [Problem Set 2](../problem-sets/problem-set-2.qmd#perturbation-theory-for-the-bose-gas) for further discussion
