...
_[TP2, 2015]_ Let's reconsider the beam splitter discussed in [Lecture 1](../notes/many-body-wavefunctions.qmd).
...
In @folling2005spatial, bosonic atoms were initially prepared in an optical lattice in a Mott state:
...
![(a) raw image (b) density of an atomic cloud following expansion from a Mott state. (c) and (d) noise correlation signal extracted from the same image.](../assets/FollingNoise.png){width=50%}
...
2. _[Harder]_. The expression for the _third order_ perturbative correction is
	...
	Show that this expression contains an _infrared_ divergence (i.e. at low momenta). This can't be fixed by appealing to the interaction potential, and is the reason we need Bogoliubov theory! See @brueckner1957bose.
