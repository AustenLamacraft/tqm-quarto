...
This also follows from taking the square modulus of @eq-product-wavefunction. The result implied by the wavefunction @eq-sym is
$$
\begin{aligned}
...
![Four possible outcomes after the passage of two bosons through a beam splitter.](../assets/HOM.png)
...
A set of labels $\{\alpha_{i}\}$ $i=1,2,\ldots N$ tells us the state of each of the particles. Thus we can write an eigenstate of $N$ _distinguishable_ particles with energy $E=\sum_{i=1}^{N}E_{\alpha_{i}}$ as
...
(We will frequently switch between the wavefunction $\varphi(\mathbf{x})$ and bra-ket notation $\ket{\varphi}$. In the latter notation the product wavefunction in @eq-product-wavefunction is written $\ket{\varphi_{1}}\ket{\varphi_{2}}$)
...
As we've just discussed, however, we should really be dealing with a totally symmetric or totally antisymmetric wavefunction, depending on whether our identical particles are bosons or fermions. The generalization of the two-particle states @eq-sym to many particles involves summing over all permutations of particle coordinates $\br_i$ with respect to the state labels $\alpha_i$. To write these down we introduce the operators of _symmetrization_ and _antisymmetrization_

$$
\mathcal{S}=\frac{1}{N!}\sum_{\pi} P_\pi, \qquad \mathcal{A}=\frac{1}{N!}\sum_{\pi} \sgn(\pi)P_\pi
$$ {#eq-sym-antisym}

Here's the definition of the various quantities in @eq-sym-antisym:
...
$$
- $\sgn(\pi)$ is the [signature](https://en.wikipedia.org/wiki/Parity_of_a_permutation) of the permutation, equal to $+1$ for permutations involving an even number of exchanges (transpositions), and $-1$ for an odd number. Note that although the same permutation can be accomplished with many different sets of transpositions, the number of transpositions is either always odd or always even.

@eq-sym-antisym allows us to write the totally symmetric and totally antisymmetric versions of @eq-distinguishable as
$$
\begin{aligned}
...
where the __occupation numbers__ $\{N_{\alpha}\}$ in the boson case give the number of particles in state $\alpha$. Note that it doesn't matter if we permute _coordinates_ or state labels $\alpha_i$ in @eq-norm. Thus the symmetric product state can also be written

$$
\ket{\Psi^{S}_{\alpha_{1}\alpha_{2}\cdots\alpha_{N}}}=\sqrt{\frac{1}{N!\prod_{\alpha}N_{\alpha}!}}\sum_\pi\varphi_{\alpha_{\pi(1)}}(\mathbf{r_{1}})\varphi_{\alpha_{\pi(2)}}(\mathbf{r_2})\cdots\varphi_{\alpha_{\pi(N)}}(\mathbf{r_N}).
$$

These normalization factors yield normalized wavefunctions _if_ the single particle state $\ket{\varphi_\alpha}$ are orthonormal (as the eigenstates of the single particle Hamiltonian are). In the fermion case each $N_{\alpha}$ is either $0$ or $1$ so the prefactor simplifies. Since the order of the $\alpha$ indices is irrelevant in the boson case, and amounts only to a sign in the fermion case, states based on a given set of single particle states are more efficiently labeled by the occupation numbers. In terms of these numbers the total energy is
...
the Slater determinant @eq-slater becomes
...
@eq-3-particle generalizes for any (odd) $N$ so that the ground state Slater determinant @eq-1d-det is proportional to
...
$$
which can be proved in a variety of ways. Proving directly that @eq-1d-fermi-gs is an eigenstate of the Hamiltonian is not easy, but can be accomplished using the identity

$$
\cot(x-y)\cot(y-z)+\cot(y-z)\cot(z-x)+\cot(z-x)\cot(x-y)=1.	\nonumber
$$

Check carefully that @eq-1d-fermi-gs is periodic and totally antisymmetric.
...
Having a many particle wave function is one thing, but what to _do_ with it? Bear in mind that @eq-1d-fermi-gs is just about the simplest fermion state you can imagine, but it's not immediately clear what it is telling us.
...
::: {#fig-1d-dist layout-ncol=2}

![A sample from the probability distribution $\lvert\Psi^A_0(z_1,\ldots,z_N)\rvert^2$ for 50 particles.](../assets/1d_fermions.png){#fig-fermion}

![Uniformly distributed points on the circle described by $\vert\Psi^{S}_0(x_{1},x_{2},\ldots x_{N})\rvert^2$. The particle positions are uncorrelated.](../assets/poisson_phases.png){#fig-poisson}

Distribution of particle positions for noninteracting fermion and boson ground states.
...
We can regard @eq-average-density as an expectation of a density operator
$$
...
Starting from the Slater determinant @eq-1d-det (i.e. _not_ from the explicit form @eq-1d-fermi-gs), show that $g(x,y)$ for the ground state of the Fermi gas is
$$
...
Find the average number of particles in a momentum state $\ket{k}$ using @eq-nbar
$$
\bar N_k = \begin{cases}
1 & |k|\leq k_\text{F} \\
0 & |k|>k_\text{F}
...
$$
Thus you can think of the single particle density matrix @eq-spdm as arising from tracing out $N-1$ particles from an $N$-particle system.
...
Starting from the Slater determinant @eq-1d-det, show that
$$
...
$$
_Almost_. Looking back at @eq-density-op, we see that the product of two density operators will contain terms involving the same particle, which are absent from $\rho_2(x_1,x_2)$.
...
There's a bit more mileage in the 1D Fermi gas yet. Consider the following Hamiltonian
$$
...
For bosons, it happens that the Hamiltonian can still be solved exactly. For now, however, we'll concern ourselves only with the limit of infinite interaction: $c\to \infty$, sometimes called the impenetrable limit. In this case, _the eigenenergies coincide with those of the free fermion problem, and the eigenstates are symmetrized verions of the corresponding fermion eigenstates_:

$$
 \Psi^S(x_1,\ldots x_N)=\Psi^A(x_1,\ldots x_N)\prod_{i<j}^{N} \exp\left[i\pi\theta(x_i-x_j)\right],
$$ {#eq-jw-trans}
...
The factor in @eq-jw-trans 

$$
\prod_{i<j}^{N} \exp\left[i\pi\theta(x_i-x_j)\right]
$$ {#eq-jw-fact}

changes sign every time two particle coordinates are exchanged, which means it turns an antisymmetric fermionic wavefunction into a bosonic one. Why does this work? The interaction in the Hamiltonian @eq-ll is equivalent to a boundary condition that may be obtained by integrating the Schrödinger equation with respect to one of the coordinates through a small interval containing another

$$
\frac{1}{2m}\frac{\partial \Psi}{\partial x_i}\Bigg|^{x_i=x_j+}_{x_i=x_j-} = c \Psi(x_i=x_j)
$$

Thus for $c\to\infty$ the wavefunction must vanish when two coordinates coincide, if the jump in the gradient is to remain finite (giving a finite kinetic energy). This is what $\Psi^A(x_1,\ldots x_N)$ does by construction, and this property is unaltered by the alternating sign factor in @eq-jw-trans. Just like that, we've solved our first interacting many body system (and with infinite coupling, no less)!

Combining @eq-1d-fermi-gs with @eq-jw-trans gives the explict form of the impenetrable Bose gas ground state
$$
\Psi_0(x_1,\ldots, x_N) = \prod_{i<j}^{N} \left|\sin\left(\frac{\pi[x_{i}-x_{j}]}{L}\right)\right|.
$$

This mapping is quite powerful, and allows us to calculate any observable of the impenetrable Bose gas in terms of free fermions _as long_ as that observable is insensitive to the presence of the factor @eq-jw-fact. Thus the average density $\rho_1(x)$ and pair distribution $\rho_2(x_1,x_2)$ of the previous section can be found in this way, but the single particle density matrix $g(x,y)$ cannot.
...
This means that the momentum distribution is _not_ given by @eq-nk. Finding $g(x,y)$ for the impenetrable Bose gas is in fact really hard. We'll see in a later lecture how to obtain some of its important features.
