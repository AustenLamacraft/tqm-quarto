...
There seems to be room for disagreement about how to define $S(\omega)$. @chaikin1995principles and @nozieres2018theory have it this way, @nazarov2013advanced have the energy axis inverted. It comes down to whether you want to think about energy going to or from the system (see later).
...
Normally we write $A(t)$ rather than $A_I(t)$, as this is the Heisenberg picture for the unperturbed problem. @eq-kubo is known as the __Kubo formula__. It expresses the response of a system in terms of the dynamics of the unperturbed system.
...
We've met an example of this relation before, see @eq-noise-asym.
...
\begin{align}
\chi_{AB}''(t) &= \frac{-i}{2}\left(\chi_{AB}(t) - \chi_{BA}(-t)\right)\\
&= \frac{1}{2}\langle\langle\left[A_I(t),B_I(0)\right]\rangle\rangle\\
&= \frac{1}{2} \left(S_{AB}(t)-S_{BA}(-t)\right).
\end{align}

::: {.callout-tip}
## Check
Verify that this function appears in dissipative response. Hint: consider the change of the expectation value of the energy E=\Tr[\rho H] due to a perturbation $\Sum_{i}\left( f_i e^{-i\omega t} + f_i^* e^{i\omega t}\right) O_i. If stuck, see Chaikin, Lubensky section 7.6.3.
...
This is the most general form of the quantum fluctuation dissipation relation, which we met before in the case of the oscilator, see @eq-FDT-asym.
