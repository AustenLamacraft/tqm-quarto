...
Solving this problem, as you've doubtless done many times, involves finding $y(t)$ given $f(t)$. Without going any further, what can we say about such a solution? @eq-sho is linear, which means that $y(t)$ depends linearly on $f(t)$. By the principle of superposition we can immediately write
...
Second, since the LHS of @eq-sho has no explicit time dependence, $\chi$ is really a function only of $t-s$. We can certainly imagine situations where this is not true. Causality, however, seems pretty watertight (even if we don't understand why).

We can interpret $\chi(t)$ as the solution of @eq-sho to a $\delta$-function force at $t=0$.
...
Since $\chi(t)=0$ for $t<0$, we can get a $\delta$-function in @eq-chi-ode by choosing a superposition of $e^{-i\omega_\pm t}$ such that $\chi'(0+)=m^{-1}$
...
The second method is to solve the equation @eq-chi-ode in the Fourier domain, where it becomes
...
The integrand has two poles in the lower half plane at $\omega_\pm$, on account of $\gamma$ being positive, which in turn follows from (or defines?) the direction of time. The fact that the frequency response is _analytic in the upper half plane_ is equivalent to causality in the real time response. This is because for $t<0$ we can close the contour in @eq-chi-FT in the upper half plane, avoiding the poles and getting zero by Cauchy's theorem. For $t>0$ we close in the lower half plane (not forgetting the - from going the other way) and use the residue theorem to give
...
Check that @eq-chi-sho satisfies @eq-kk.
...
The model was originally studied by Horace Lamb in a 1900 paper titled ["On a Peculiarity of the Wave‐System due to the Free Vibrations of a Nucleus in an Extended Medium"](https://londmathsoc.onlinelibrary.wiley.com/doi/abs/10.1112/plms/s1-32.1.208). Lamb wasn't talking about the nucleus of the atom — that hadn't yet been discovered — but of any body coupled to a continuum.
...
@eq-ret corresponds to a 'zero temperature' environment. Finite temperature corresponds to an 'incoming' wave describing the thermal occupation of the Fourier modes of the string, which would cause Brownian motion of the oscillator. The energy scale of this motion is $k_\text{B}T$.
...
A differential equation driven by noise like @eq-sho-noise is known as a __Langevin equation__ in physics and as a __stochastic differential equation__ (SDE) in mathematics.

::: {.callout-note}

Actually, no self-respecting mathematician would write something like @eq-sho-noise, implying as it does that the instantaneous acceleration of the oscillator is infinite. Physicists, on the other hand, know that this is just an approximation caused by setting certain time scales to zero.

:::

The mathematical way to write an equation like @eq-sho-noise is as a pair of first order SDEs

$$
\begin{aligned}
dy &= \frac{p}{m} dt\nonumber \\
dp &= -\left[m\omega_0^2 y +\frac{\gamma p}{m}\right]dt +\sqrt{2\gamma k_\text{B}T}dW,
\end{aligned}
$$
...
Now what about _time dependent fluctuations_? This is what was calculated in @eq-power-final for the classical damped oscillator. A natural candidate is
...
Many of the properties of this function are most easily understood from a __spectral representation__, which is obtained by inserting a complete set of energy eigenstates between $y(0)$ and $y(t)$ in @eq-q-noise. This gives
...
There seems to be room for disagreement about how to define $S(\omega)$. @chaikin2000principles and @nozieres2018theory have it this way, @nazarov2013advanced have the energy axis inverted. It comes down to whether you want to think about energy going to or from the system (see later).

:::

At zero temperature $\beta\to\infty$ and we can see that $S(\omega<0)=0$.

Let's evaluate @eq-s-spectral for the oscillator. The matrix elements are
...
is the Bose distribution function. @eq-qSHO-noise shows the predicted asymmetry between positive and negative frequencies. We can check that
...
With a continuum of modes, as in the oscillator coupled to a string, $S(\omega)$ can have a smooth (not just $\delta$-function like) behaviour. Nevertheless, the basic relation @eq-noise-asym is still obeyed.
...
where $y$ is written in terms of the normal modes as in @eq-y-mode. The equation of motion for the modes is
...
Note the imaginary infinitesimal in the denominator. This is a notational device to remind us that, even though the normal mode has no damping, a causal (retarded) response is analytic in the upper half plane of $\omega$. The response of $y(\omega)$ is then obtained from @eq-y-mode
$$
...
but we also know the LHS from the classical calculation @eq-chi-sho.

@eq-qSHO-noise-gen and @eq-chi-modes are rather similar. We can get an explicit relation by using the formula
...
with $\chi''(-\omega)=-\chi''(\omega)$. Comparing with the classical answer @eq-chi-sho allows us to to find the $|c_k|$ without having to go through the trouble of finding the normal modes explicitly (although we'd be using our knowledge of their density of states).
...
Check that @eq-FDT-asym reduces to @eq-classical-fdt in the classical limit. It may help to (just this once) put $\hbar$ back in and then let it go to zero.
...
Let's compare with our previous expression for the energy dissipated, @eq-class-diss. We see we have agreement for $n_\text{B}(\omega)\to 0$. This corresponds to the energy of the transition being much larger than the thermal energy $\hbar\omega\gg k_B T$.
...
Here $\theta(t)$ is the step function
$$
\theta(t)=\begin{cases}
1 & t\geq 0\\
0 & t<0
...
included so that the response function is causal. Normally we write $A(t)$ rather than $A_I(t)$, as this is the Heisenberg picture for the unperturbed problem. @eq-kubo is known as the __Kubo formula__. It expresses the response of a system in terms of the dynamics of the unperturbed system.
...
Show this using the cyclic property of the trace.
...
We've met an example of this relation before, see @eq-noise-asym. We now use it to relate $S_{AB}(\omega)$ to the function
$$
\chi''_{AB}(t)\equiv \frac{1}{2}\langle\langle\left[A_I(t),B_I(0)\right]\rangle\rangle,
$$

which differs from the full causal response function in @eq-kubo because it is lacking the step function that makes the response causal. It turns out that this is the dissipative dissipative part of the response.
...
<!-- We write this as $\chi_{AB}(t) = i\theta(t)\langle\langle\left[A_I(t),B_I(0)\right]\rangle\rangle$, where $\theta(t)$ is the step function
...
Consider now the dissipative part of $\chi_{AB}(t)$
...
Fourier transforming and using the relation @eq-gen-asym gives

$$
S_{AB}(\omega) = 2\chi_{AB}''(\omega)\left[n_\text{B}(\omega)+1\right].
$$ {#eq-fdt-gen}

This is the most general form of the quantum fluctuation dissipation relation, which we met before in the case of the oscillator, see @eq-FDT-asym.
...
Use the spectral representation to prove the fluctuation dissipation relation @eq-fdt-gen.
...
$$
\langle\langle \rho_\bq(t)\rangle\rangle = -\frac{1}{L^3} \int_{-\infty}^t  \chi_{\rho}(\bq,t-t') V_\bq(t')\,dt',
$$

where the density response function $\chi_\rho(\bq,t)$ is
...
Some texts (e.g @nozieres2018theory) omit the $2\pi$ in the definition @eq-S-def. I prefer to keep it so that $S_\rho(\bq,t) = \langle\langle \rho_\bq(t)\rho_{-\bq}(0)\rangle\rangle$.
...
The inverse of the compressibility is also known as the [bulk modulus](https://en.wikipedia.org/wiki/Bulk_modulus), often denoted by $\kappa$. Using @eq-beta-deriv the compressibility can be related to the change in the energy density of a system in an external potential $V(\br)$. The potential causes the density to deviate from its value in the uniform system. This deviation can be found by considering the energy density to quadratic order in the density deviation 

$$
\epsilon(\rho+\delta\rho) = \frac{1}{2\beta\rho^2} \left[\delta\rho\right]^2 + V(\br)\delta\rho.
$$

Minimizing this expression with respect to $\delta\rho$ gives

$$
\epsilon(V(\br)) = - \frac{\beta\rho^2}{2} \left[V(\br)\right]^2
$$ {#eq-v-quad}

We now compare this expression with the result of second order perturbation theory for the energy due to the perturbation

$$
\sum_j V_0 \cos(\bq\cdot \br_j) = \frac{V_0}{2}\left[\rho_\bq+\rho_{-\bq}\right]. 
$$

The standard expression of second order perturbation theory (the first order vanishes) gives
$$
E^{(2)} =\frac{V_0^2}{2} \sum_{n\neq 0} \frac{|\braket{0|\rho_\bq|n}|^2}{E_0-E_n} =-\frac{V_0^2}{2}\int_0^\infty \frac{S(\bq,\omega)}{\omega}\frac{d\omega}{2\pi}.
$$ {#eq-2nd-order}
...
Confirm @eq-2nd-order. Note that for a translationally invariant system, energy eigenstates are eignestates of momentum. If the ground state $\ket{0}$ has zero momentum, then $\rho_\bq\ket{0}$ has momentum $\bq$ and only has nonzero matrix elements with eigenstates of momentum $\bq$. For eigenstate $\ket{n}$ with momentum $\bq$ there is one with momentum $-\bq$ of the same energy (by parity symmetry).

:::


Comparing this expression with @eq-v-quad, using the average value of $\langle\left[V(\br)\right]^2\rangle = V_0^2/2$
...
The compressibility sum rule is often written in terms of the speed of sound $c = (\beta m \rho)^{-1/2}$ as
...
where $\omega(\bq)$ is the dispersion relation of the collective excitations. In reality, the $\delta$-function is never infinitely sharp, but has a finite width on account of interactions between excitations causing scattering and / or decay. In the approximation @eq-sma, often known as the __single mode approximation__, the f-sum rule tells us that
