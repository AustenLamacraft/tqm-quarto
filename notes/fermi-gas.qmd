...
The definition @eq-fdef implied a certain quantization axis for spin. To write things in an invariant way, we should think of the occupation number $N(\bk)$ as a $2\times 2$ matrix that can describe an arbitrary spin orientation, with elements
...
The message to take away from this calculation is not the detailed functional form of @eq-ffinal, but the fact that the interaction between quasiparticles in an interacting Fermi gas is defined in terms of a pair of functions
