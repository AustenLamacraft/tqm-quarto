...
The elastic chain was a model defined in terms of the displacements $u_j$ and momenta $p_j$ of $N$ particles satisfying $[u_j,p_k]=i\delta_{jk}$. In this lecture we are going to consider models where the individual degrees of freedom are quantum mechanical _spins_, defined by a vectors of operators $\mathbf{s} = (s^x, s^y, s^z)$ at each site, obeying the usual spin commutation relations $[s^a,s^b]=i\epsilon_{abc}s^c$. Such _spin models_ are often used as quantum mechanical models of magnetic materials. Despite the obvious differences, we will see that there are a number of similarities between the elementary excitation of the two types of system.

::: {.callout-note }

Remember that instead of the components $(s^x, s^y, s^z)$ it's often useful to work with $s^\pm = s^x\pm i s^y$ with commutation relations
...
$s^\pm$ are spin _raising_ and _lowering_ operators.

:::

Reading: @auerbach2012interacting.
...
One spin-1/2 is described by a state $\ket{\psi}$ in a two-dimensional Hilbert space. It's common to choose as a basis the eigenstates of $s^z$. If we denote these as $\ket{\pm}$ with $s^z\ket{\pm} = \pm \frac{1}{2}\ket{\pm}$, we write
$$
\ket{\psi} = \psi_+\ket{+} + \psi_-\ket{-}=\begin{pmatrix}
\psi_+ \\
\psi_-
\end{pmatrix}.
$$
...
$$
\ket{\sigma_1}\ket{\sigma_2}\cdots \ket{\sigma_N}, \qquad \text{where } \sigma_j=\pm. 
$$

We met product states as explicit wavefunctions in [Lecture 1](many-body-wavefunctions.qmd). The abstract structure is called a [tensor product](https://en.wikipedia.org/wiki/Tensor_product), normally denoted

$$
\ket{\sigma_1}\otimes\ket{\sigma_2}\otimes\cdots \otimes \ket{\sigma_N}
$$
...
Since @eq-hberg is supposed to be a model for a magnet, we might try the state with all spins aligned
...
@eq-spin-lowered is a superposition of terms with one spin flipped. If we continue the process to explore the whole multiplet, we will at each state end up with a constant amplitude superposition of all states with two spins flipped (for $S^z=N/2-2$), three flipped (for $S^z=N/2-3$) and so on.
...
What about just one of the states in the superposition @eq-spin-lowered?
...
We can easily diagonalize $H$ in this space by noting that it corresponds to a circulant matrix. As we saw in [Lecture 3](elastic-chain.qmd), the eigenstates are plane waves
...
Note that the dispersion relation is periodic, as in the case of the elastic chain, but is now quadratic, rather than linear, at small $\eta$. $\eta=0$ corresponds to the state @eq-spin-lowered, and indeed the energy $\omega(0)=0$ in that case.
...
Confirm that these expressions reproduce the spin commutation relations @eq-spin-ladder. Note that since

$$
\left[\adop\aop,\adop\right] = \adop,\qquad \left[\adop\aop,\aop\right] = -\aop,
$$

we have for any function of the _number operator_ $N\equiv \adop\aop$

$$
f(N)\adop = \adop f(N+1),\qquad f(N)\aop = \aop f(N-1).
$$

:::

[This problem](../problem-sets/problem-set-1.qmd#from-schwinger-bosons-to-holstein--primakoff) from Problem Set 1 should demystify why this works to some extent. A rough understanding is as follows. $s^{\pm}$ and $\aop$, $\adop$ evidently have something in common in that they shift us up and down a ladder of states. That's why the relation between $s^z$ and the number of quanta in the oscillator is so simple. The oscillator ground state corresponds to $\ket{s,s}$.
...
$$
and is the ground state for $J<0$. We could continue to analyze @eq-h2-aop in terms of the operators $\adop_j$ and $\aop_j$ see [this problem](../problem-sets/problem-set-1.qmd#spin-waves-from-bogoliubov-transformation) from Problem Sheet 1, but instead we're going to write @eq-hp-approx as
...
where we are dropping terms that are lower order in $s$. @eq-harmonic has the form of an harmonic chain, albeit one with a rather strange looking mass term. We can diagonalize it by using the Fourier expansion of the position and momentum as before
...
in agreement with @eq-dispersion for $s=1/2$. Incidentally, in order that the ground state energy is $NJ s^2$ with no correction at order $s$, the ground state energy of the oscillators must cancel the second term of @eq-harmonic. In other words,
$$
\sum_{|n| \leq (N-1)/2} \sin^2(\eta_n/2)\bra{0}\left[q_n q_{-n} + \pi_n\pi_{-n}\right]\ket{0}=\frac{N}{2}.
$$ {#eq-h2-gs}

The harmonic approximation applies close to the ferromagnetic state, where there are few oscillator quanta. If the typical values of $\adop_j\aop_j$ are small — we've seen $\adop_j\aop_j=0$ in the ground state — the approximation $\sqrt{1-\frac{\adop_j\aop_j}{2s}}\sim 1$ that gave rise to @eq-hp-approx is justified, and neglecting anharmonic terms is reasonable. 

What about the case of antiferromagnetic coupling ($J>0$)? Even though the generalization of @eq-afm

$$
\ket{\text{AFM}}_s \equiv \ket{s,s}_1\ket{s,-s}_{2}\cdots \ket{s,s}_{N-1}\ket{s,-s}_{N},
$$

is not a ground state, it might be close. We can't apply the approximation @eq-hp-approx directly, however, because this state has zero quanta on the odd sites, but $2s$ quanta on the odd sites. To apply our approximation to states close to $\ket{\text{AFM}}_s$ we rotate every other spin through $\pi$ about the $y$ axis, so that
...
This is rather different to the ferromagnetic case. Note that it vanishes both $\eta=0$ and at the Brillouin zone boundary $\eta=\pi$, and is linear in the vicinity of both points, compared with the quadratic behaviour of the ferromagnet. The reason for the difference is clear on comparing @eq-h2-fm and @eq-h2-afm. In the first case both the position and momentum terms vanish at $\eta=0$, while in the second only the position term vanishes here, with the momentum term vanishing at $\eta=\pi$.
...
The halving of the period of the dispersion @eq-afm-disperion is due to the antiferromagnetic order doubling the size of the unit cell. Although the exact result  has the same feature, there is in fact no antiferromagnetic order in the chain, as we'll see next.
...
This commutes with the harmonic Hamiltonian @eq-h2-fm, and is zero in the ground state (c.f. @eq-h2-gs). In fact, this is an exact statement not restricted to the harmonic approximation: the total number of Holstein--Primakoff bosons is conserved. The same is not true for the antiferromagnetic case @eq-h2-fm. If we had written this Hamiltonian in terms of the operators $\adop_j$, $\aop_j$, we would have found terms involving $\adop_j\adop_{j+1}$ and $\aop_j\aop_{j+1}$ that create pairs of bosons (see [this problem](../problem-sets/problem-set-1.qmd#spin-waves-from-bogoliubov-transformation)).

Next we use the oscillator coordinates just as in [Lecture 3](elastic-chain.qmd). Since we want to express @eq-h2-afm in terms of ladder operators, we have to adapt our definition accordingly. For $\eta\neq 0, \pi$ the operators
...
This puts the Hamiltonian @eq-h2-afm in the desired form, so we know that the ground state satisfies $\aop_\eta\ket{\text{GS}}=0$

To evaluate @eq-spin-deplete, we write it in terms of $\adop_\eta$, $a_\eta$ to give
...
![The ground state energy of the Heisenberg chain for $L=26$ as a function of $\eta$, compared with the des Cloiseaux--Pearson dispersion @eq-dcp.](../assets/Heisenberg_model_dispersion.png)
...
to work. Acting on @eq-2-mag with the Hamiltonian gives the eigenvalue equation for the energy $E = E_0 +\epsilon$
...
The first of @eq-2-part is solved by
...
There's a clever way to do this. First, even though we know $c_{j,j}=0$ is really zero, we extend the first of @eq-2-part to $j=k+1$, and this equation then involves a 'fake' $c_{j,j}$. The advantage of this is that we can then extend the form @eq-2-wave to $j=k+1$ and $j=k$. For this to work, the correct equation for $j=k+1$ -- the second of @eq-2-part -- must be consistent with the first. This gives the condition

$$
-c_{k+1,k} + \frac{1}{2}\left(c_{k,k} + c_{k+1,k+1}\right) = 0.
$$

Using the form @eq-2-wave gives a relationship between $c_1$ and $c_2$
...
We also require that $c_2=0$ in @eq-2-wave, otherwise the eigenstate doesn't decay. From @eq-c-ratio we see that
...
### The Anderson Tower {#sec-anderson}
...
Since it seems to be hard to solve the Heisenberg model, we simplify it in the following way (this discussion follows 
 @lhuillier2006frustrated). Introduce the Fourier components of the spin
...
gives all the wavevectors in the Brillouin zone. It's easier to assume $N$ is even here, to accommodate antiferromagnetic order. In terms of $\tilde{\mathbf{s}}_{\boldsymbol{\eta}}$, the $d$-dimensional version of the Heisenberg Hamiltonian @eq-hberg on the square / cubic / hypercubic lattice can be written
...
with degeneracy $2S+1$. In the second case, reducing $S_A\to \cN/4 - 1$ means that $\Delta\mathbf{S}_A^2 = -\frac{\cN}{8}$. From @eq-hab we see that this is going to cause the energy to increase by an amount of order $J$, while in the first case our excitations were of order $J/\cN$. The two kinds of excitations only have comparable energies when $S$ is $O(\sqrt{N})$. For smaller values of $S$ the "type 1" states have a lower energy
...
Evaluate the expressions @eq-n-idents and see that they are fulfilled in the limit $\cN\to\infty$.
