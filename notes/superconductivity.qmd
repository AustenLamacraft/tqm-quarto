...
So BCS got the Nobel prize (which conveniently can be split three ways) and their theory become one of the pillars of condensed matter physics. In this lecture we'll discuss BCS theory using the model Hamiltonian @eq-model with attractive short-ranged interactions. This is not a particularly realistic model of the combined phonon-mediated attraction and screened Coulomb repulsion, but displays the same physics. It _is_ a good model for pairing of ultracold fermionic atoms, first observed in 2004 @regal2004observation.

## BCS Theory

### The BCS Wavefunction

In momentum space our Hamiltonian @eq-model has the form
...
where the numbers $n_{\bp}^P$ are either $0$ or $1$ (why?). Note that in writing @eq-pair-fermi the number of each species is assumed to be exactly $N/2$. Restricting ourselves to states of this form means that
$$
...
\downarrow}$ and its conjugate $\bop_\bp=\aop_{-\bp,\downarrow}\aop_{\bp\uparrow}$, that respectively create and destroy a $\left(\bp\uparrow,-\bp\downarrow\right)$ pair. Because our pair ansatz @eq-pair-fermi only includes amplitudes for a given $\left(+\bp,-\bp\right)$ pair of momenta having either none or two fermions, we can compute expectations using the pair Hamiltonian
$$
...
Now, can we solve @eq-pair-h? This may now look like a quadratic problem, but the pair operators $\bop_\bp$, while commuting at different momenta
$$
...
corresponding to @eq-pair-fermi but with a set of coefficients $c_{\{n^P_\bp\}}$ that factorizes.
Finding the variational energy of @eq-bcs-cons is still a tricky problem. For instance, what is the
...
Finding the average number of pairs $\langle n^P_\bp\rangle$ in @eq-bcs-cons is however not
...
This is a superposition of states with different total number of particles. Apart from the overall normalization, the projection of @eq-bcs onto a fixed number $N$ of particles corresponds exactly to @eq-bcs-cons if $c_\bp=v_\bp/u_\bp$. Since @eq-bcs is a product of factors corresponding to each
momentum separately,  $\langle n^P_\bp\rangle$ is easily found to be $v_\bp^2$. The total variational
energy of this state is

$$
\braket{\text{BCS}}{H_{\text{pair}}}{\text{BCS}}=2\sum_\bp \epsilon_{\bp}|v_\bp|^2+\frac{U_0}{V}\sum_{\bp,\bp'}u^*_\bp v_
\bp u_{\bp'}v^*_{\bp'}.
$$ {#eq-bcs-en}
...
In the thermodynamic limit, we might as well work with the non-conserving form @eq-bcs. To fix the number of particles, we should introduce a chemical potential into the Hamiltonian. Note that we didn't have to do this with the Bogoliubov theory as we fixed the number of particles in the condensate $N_0$.

There is an interesting alternative interpretation of the pair Hamiltonian @eq-pair-h. Acting within
...
This representation is sometimes called __Anderson spins__. In this way we can write @eq-pair-h as a spin chain (up to a constant)

$$
\overbrace{H_{\text{pair}}-\mu N}^{\equiv H_\mu}=2\sum_\bp \xi({\bp})S_\bp^z+\frac{U_0}{V}\sum_{\bp,\bp'}S^+_\bp S^-_{\bp'},
$$

where we have included the chemical potential by defining $\xi_\bp\equiv\epsilon(\bp)-\mu$. If we parameterize $\left(v_\bp,u_\bp\right)$ as $(\cos(\theta/2)e^{i\varphi/2},\sin(\theta/2)e^{-i\varphi/2})$
then the variational energy @eq-bcs-en has the form 

$$
\braket{\text{BCS}}{H_\mu}{\text{BCS}}=\sum_\bp \xi_\bp\cos\theta_\bp+\frac{U_0}{4V}\sum_{\bp,\bp'}\sin
\theta_\bp\sin\theta_{\bp'}\cos\left(\varphi_\bp-\varphi_{\bp'}\right).
$$ {#eq-spin-en}

The interpretation of @eq-spin-en is the following. The first term tends to align the spins with the z-axis in the - direction for $\xi_\bp<0$ and in the + direction for $\xi_\bp>0$. On the other hand, the
...
It remains to actually minimize the energy @eq-bcs-en to determine the $u$'s and $v$'s, or
...
the increase in kinetic energy that comes from smearing the step, see @eq-ke. Clearly all of the
angles $\varphi_\bp$, describing the angle in the x-y plane, should be equal. Taking  the extremum of
@eq-spin-en with respect to the angles $\theta_{\bp}$ gives the condition

![Anderson spin configurations and the associated distribution functions for the free fermi gas (top) and the BCS state (bottom).](../assets/wall.png){width=70%}
...
where $\Lambda$ is the UV cut-off (we will shortly identify it with the Fermi energy), and $\nu(\mu)$ is the density of states per spin component at the chemical potential. This shows that no matter how small the attraction $U_0<0$, there will always be a solution of @eq-cont-sc with finite $\Delta$. This is the essence of the __Cooper phenomenon__. It should be compared with the
situation in which there are not a macroscopically large number of particles present. In that case $\mu=0
$, so that $\xi_{\bp}=\epsilon(\bp)>0$. Then the right hand side of @eq-cont-sc has no divergence at
...
from the first quantized form of the number conserving wavefunction @eq-bcs-cons
...
large compared to the separation between pairs. In this limit the antisymmetrization operation, required by the exclusion principle, plays a dominant role, as we have seen. When the pair wavefunction $\varphi(\br)$ has a much smaller extent than the typical separation between pairs, we can expect that the antisymmetrization operation in @eq-bcs-first is not too important, as two fermions of the same
type rarely overlap. In this limit, any given momentum state has a low average occupancy, and the hardcore constraint @eq-hardcore does not play a significant role. Then @eq-pair-h can really be thought of as a Hamiltonian for isolated pairs, with the corresponding binding energy. The resulting wavefunction is then essentially a Gross-Pitaevskii state of molecules. This all suggests that the BCS state is smoothly connected to the Bose--Einstein condensate (BEC), though the __BCS-BEC crossover__.

To see how this works in detail we need to address the issue of regularizing @eq-gap-eq. The details are given in the Appendix, but the conclusion is very simple. The gap equation can be written in terms of the _scattering length_ $a$ (if you haven't met scattering theory before, the meaning of this quantity is explained in the Appendix too) of the two-particle potential instead of interaction strength $U_0$
...
the Fermi energy, and the chemical potential is just equal to the Fermi energy $E_F=k_\text{F}^2/2m$. The integral in @eq-reg can then be done explicitly to give the gap
...
The two  equations @eq-reg and @eq-mu-eq are conveniently cast in the dimensionless
form
...
![Variation of gap and chemical potential accross the BCS (left, weak attraction) to BEC (right, strong attraction).](../assets/BCS-BEC.png){width=70%}
...
corresponding to the momentum state $\bp$ certainly containing one particle with spin $s$, and the $\left(-\bp,-s\right)$ state certainly being empty. The result is an eigenstate of momentum and spin, although it is not of course an energy eigenstate of $H_\mu$. Nevertheless, we can regard it as an approximate variational excited state, since it is orthogonal to $\ket{\text{BCS}}$. To evaluate its energy, notice that if we choose $s=\uparrow$ then the $\left(\bp,\uparrow\right)$ state is occupied and $\left(-\bp,\downarrow\right)$ empty, and we have
$$
\adop_{\bp
\uparrow}\adop_{-\bp\downarrow}\ket{\bp,\uparrow}=0,\qquad \aop_{-\bp
\downarrow}\aop_{\bp\uparrow}\ket{\bp,\uparrow}=0
$$

This means that the $\bp$, $-\bp$ pairs no longer contribute when $\tilde H_{\text{int}}$ (@eq-bcsH) is applied. The level is said to be `blocked', and so the effect of the attractive interaction is reduced. To find the energy of this state we have to take into account the kinetic energy as well as the loss of attractive interaction energy, see @eq-spin-en
...
The gap equation @eq-gap-eq has an ultraviolet divergence -- which we also encountered in the Bose gas -- that is really a two body effect that arises from our model $\delta$-function potential. To see why this is so, consider the two particle Schrödinger equation
...
The solution of @eq-gen-gap-eq is in general very difficult, but it can be greatly simplified if we assume that the range of the potential $U(\br)$ is the shortest length scale in the problem. With this assumption we can write @eq-gap-eq
...
By comparing the Fourier transform of @eq-inhomog at $E=0$ and @eq-gap-reg, we can conclude
...
Which coincides with @eq-reg (we drop the subscript $0$ from now on, as we never need to discuss the high momentum behaviour of $\Delta_\bp$ again).
...
The excitation energy of this state is twice the 'magnetic field' each pair experiences $2E(\bp)$, see @eq-heff. The spin interpretation allows us to see easily that this state contributes $-u_\bp v_\bp$ to the self consistent equation, and occurs with probability $n_{\uparrow}(\bp)n_{\downarrow}(-\bp)$. The overall result is thus
...
where $\nu(E_F)$ is the fermi surface density of states per spin, and we have left out the upper cut-off. This expression needs to be regularized as in the zero temperature case considered before. The resulting $\Delta(T)$ varies from $\Delta(0)=\Delta_{\text{BCS}}$ given by @eq-bcs-gap, to zero at
...
There is a small problem. If we continue the calculation through the crossover, the temperature at which a non-zero $\Delta$ develops continues to increase indefinitely on the BEC side @de1993crossover
...
![$T_c$ in the mean-field theory (solid line), compared with the result of a treatment that smoothly interpolates to $T_{\text{BEC}}$ (dashed). Reproduced from @de1993crossover.](../assets/EngelbrechtTc.png){width=70%}
