...
$$
\begin{gather}
 \frac{1}{\sqrt{(N-1)!}}\sum_m \psi_m(X) \sum_P \psi_{1}(x_{P_1})\cdots \psi_{m-1}(x_{P_{m-1}})\psi_{m+1}(x_{P_{m}})\cdots\psi_{N}(x_{P_{N-1}}) \\
=\frac{1}{\sqrt{(N-1)!}}\sum_P\psi_{1}(x_{P_1})\psi_{2}(x_{P_2})\cdots\psi_{N}(x_{P_N})
\end{gather}
$$

with $x_N=X$. But this just corresponds to applying the prescription @eq-freeze to the original wavefunction @eq-N-prod. Since this works for all product states, and since any state is a superposition of product states, it works more generally.
