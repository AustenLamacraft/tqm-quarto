...
Reading: @coleman2015introduction, Chapter 2.
...
Show that in terms of the Fourier modes the Hamiltonian @eq-H-chain is
$$
...
$$ {#eq-sho}
we can read off the dispersion relation @eq-dispersion. The decomposition @eq-r-and-i amounts to working with the _real_ amplitudes of sine and cosine waves. Other options are possible.
...
Satisfy yourself — without doing any calculations — that $a(t)$ defined by @eq-a-def goes clockwise in the complex plane.
...
$$
Inverting the definition @eq-many-a-def gives
$$
\begin{align}
...
\end{align}
$$
and inserting into the Hamiltonian @eq-h-diag gives
$$
...
In the quantum theory, the variables $\aop$, $\adop$ satisfy $[\aop,\adop]=1$ and the Hamiltonian @eq-sho takes the form
$$
...
The quantization of the chain is then a piece of cake. We have a system of oscillator variables satisfying $[\aop_m,\adop_n]=\delta_{mn}$, and writing the Hamiltonian @eq-h-diag in terms of these variables, paying attention to the order, gives
$$
...
$$
to exist. The separation of the $\eta_n$ values is $2\pi/N$, so as $N\to\infty$ the sum in ground state energy @eq-H0 can be replaced with an integral according to the rule
$$
...
$$
(This differs from @eq-p-exp by $\pi_n\to \pi_n/\sqrt{N}$.) we have $\left[u_m,\pi_n\right]=\delta_{mn}$.
...
\omega(k) = c|k|,
$$
where $c=\sqrt{\frac{\kappa}{\rho}}$ is the speed of sound. Note that the continuum limit has lead to a linear dispersion relation. This is reasonable: the lattice was the origin of the periodic dispersion relation @eq-dispersion, and it has now disappeared.

Transcribing the definition of the oscillator variables @eq-many-a-def gives
$$
\begin{align}
...
$$
This is finite, even in the $N\to\infty$ limit, on account of the exponential tail of @eq-bose. Remember that this was the problem that the quantum theory was introduced to solve. At low energies $n_{\text{B}}(\omega) = \frac{1}{\beta\omega}$ and the contribution of each mode is
$$
...
\end{multline}
$$ {#eq-u-var}
The dispersion relation @eq-dispersion makes this integral a little difficult.

![The integrand in @eq-u-var for $j-k=20$. What form does it take for $\left|j-k\right|^{-1}\lesssim\eta\lesssim \pi$?.](../assets/disp_fluct.png)
...
where we have introduced $\ell_\text{osc}=\left(km\right)^{-1/4}$, the natural length scale of an oscillator. If you find this derivation a bit vague, an evaluation of the integral @eq-u-var is given in the @sec-u-var.

:::

The result @eq-log-fluct shows that the uncertainty in the separation of two masses in the chain increases with their separation. This implies that the ground state of the chain is _not_ a crystal, but more closely resembles a fluid. It turns out that this conclusion depends strongly on the dimensionality of the system. In higher dimensions, crystalline ground states _do_ exist (thankfully).
...
$$ {#eq-struct}
Upon substituting the mode expansion @eq-mode-exp for the displacements, we are left to evaluate expressions of the form
$$
...
Deploying this strategy for @eq-struct gives
$$
\begin{equation}
\begin{split}
...
![Numerical evaluation of $\bra{0} \rho_q \rho_{-q} \ket{0}$ for $\ell_\text{osc}/a=0.2$, $N=51$. Note that the second Bragg peak is hardly visible.](../assets/struct_fact.png)

The second exponent modifies the classical prediction of Bragg peaks. We have encountered the expression in exponent before in @eq-u-var. Taking the $N\to\infty$ limit with $\left\|j-k\right\|\gg 1$ as before, we find that in the vicinity of the first Bragg peak at $q=2\pi/a$ the summand has the form
$$
...
$$ {#eq-dft}
where $\eta_n\equiv \frac{2\pi n}{N}$, and $n = -(N-1)/2,\ldots, (N-1)/2$. The key to inverting @eq-dft is the observation
$$
...
$$ {#eq-idft}
A more democratic definition would have $1/\sqrt{N}$ in both definitions @eq-dft and @eq-idft. This would allow us to regard the DFT as a basis change to an orthonormal basis of vectors $e^{(n)}_j = \frac{e^{i\eta_n j}}{\sqrt{N}}$, and both the DFT and its inverse would be unitary transformations.
...
The analog of the identity @eq-dft-ident is
$$
...
Finally we arrive at the Fourier transform, where we take $L\to\infty$, so that the inverse transform in @eq-ft-series becomes an integral too
$$
\begin{align}
...
$$
With practice, you will find you are able to write down the right hand sides of expressions like @eq-ft-examples without too much difficulty.
...
Practice writing out @eq-ft-examples in $d$ dimensions.

:::

### Evaluating @eq-u-var {#sec-u-var}
...
$$
When all factors are replaced, this yields @eq-log-fluct.
